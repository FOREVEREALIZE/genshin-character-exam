<script lang="ts">
    ...
    const {item}: { item: Character } = $props();
...
<div
        class={`flex flex-col items-center aspect-square justify-center relative rounded-lg overflow-clip cursor-grab active:cursor-grabbing shadow bg-neutral-800 ${isDragging.current ? 'opacity-70 scale-105' : 'hover:scale-110 hover:rotate-2 transition-transform duration-150'}`}
        {style}
