...
>
    <main class="flex flex-col items-center justify-start w-full h-full bg-neutral-950 gap-8 py-6">
        <div class="w-full max-w-5xl flex items-center justify-between px-1">
            ...
            <div class="flex items-center gap-4">
                <button onclick={resetAll}
                        class="text-sm bg-red-600 hover:bg-red-500 active:bg-red-700 transition-colors text-white px-3 py-1 rounded font-medium">
                    Reset All
                </button>
            </div>
        </div>
        ...
        <section class="w-full max-w-5xl">
            ...
            <Droppable id="drawer" class="grid gap-2"
                       boxStyle="grid-template-columns: repeat(auto-fill,minmax(72px,1fr)); min-height:90px; border:1px dashed #555; padding:8px; border-radius:8px;">
                    ...
                    <tr>
                        ...
                        {#each ELEMENTS as el}
                            <td class="align-top p-1 border border-white/30">
                                <Droppable id={cellId(el, weapon)} class="grid gap-1"
                                           boxStyle="grid-template-columns: repeat(auto-fill,minmax(56px,1fr)); min-width:120px; min-height:60px;">
                                    {#each itemsInCell(el, weapon) as item (item.id)}
                                        <Draggable item={item} showAnswers={showAnswers}
                                                   isCorrect={item.element === el && item.weapon === weapon}/>
                                    {/each}
                                </Droppable>
                            </td>
                        {/each}
